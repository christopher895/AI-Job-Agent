...
- **Duplicate a resume** — the dashboard card's ⋯ menu copies a settled resume (`POST /api/resume/:id/duplicate`): same JD, markdown, suggestion history and rendered PDF land in a new row titled "… (copy)", so a variant can be edited without re-running the pipeline. Only `ready`/`awaiting_review` rows can be copied — a `pending` one has no stable content and the copy would have no pipeline to finish it. Answers come along only if they had finished generating
- **Project links in the PDF** — a project's link (the `link` field on `/resume/master`, emitted as its own line under the project header in Markdown) renders as `Name - domain, tech` with the domain hyperlinked. `displayUrl()` strips protocol/`www.`/trailing slash for display, `hrefUrl()` adds a scheme so a schemeless link still opens, and `isLinkLine()` accepts bare domains — the Link field is free text, so "swimvolt.com" is what actually gets typed. The link was parsed into the doc model but never emitted into the LaTeX before, so it silently vanished from every PDF; `test:tex-escape` (fast gate) guards the helpers and `test:render-pdf` guards the rendered output
...
| Page | Purpose |
|---|---|
| `/` | History dashboard — all generated resumes, listed by company/role/date, with download + edit links; the per-card ⋯ menu duplicates or deletes one |
...
tailored_resumes (
  ...
  critic_score  int,            -- vestigial: scored by the removed critique loop; always null on new rows,
                                -- no longer read or displayed anywhere (column retained, never dropped)
